...
from food.food_service_producer import prepare_food
...
consumer.subscribe(['order.updates', 'food.updates'], listener=kafka_util.RebalanceListener())
...
def handle_confirmed_order(event, key, event_id, event_name, version):
    logger.debug(f"In handle_confirmed_order: {event_name}, {event}")
    try:
        ...
        logger.error(f"Validation error in handle_confirmed_order: {e}")
        kafka_util.send_to_dead_letter_queue(key, event, f"Validation error in handle_confirmed_order: {e}", producer)
...
EVENT_HANDLERS = {
    'order_confirmed': handle_confirmed_order
}
